...
function ReadTAS(tas)
    local frames={}
    local l = 0
    for line in tas:gmatch("([^\n]*)\n?") do
        l = l + 1
        if l > 1 then
            local inp = 0
            ...
            frames[table.getn(frames) + 1] = inp
        end
    end
    return frames
end
...
-- local framerule = 539
-- local framerule = 2135
local framerule = 1645

function DisplayObjects()
    for i=1,table.getn(DISPLAY1),1 do
    	gui.text(0,50 + (i*8),DISPLAY1[i][1],DISPLAY1[i][2])
    end
    for i=1,table.getn(DISPLAY2),1 do
    	gui.text(0,100 + (i*8),DISPLAY2[i][1],DISPLAY2[i][2])
    end
end
...
function RegisterDisplays()
	for enemy=0,5,1 do
        ...
        DISPLAY1[enemy+1] = { string.format("%d: %s", enemy, ObjectNames[memory.readbyte(Enemy_ID + enemy)] or ""), color }
	end
	for enemy=0,5,1 do
		local color = "#ffffff"
        local timerval = memory.readbyte(Cannon_Timer + enemy)
        DISPLAY2[enemy+1] = { string.format("%d: %d", enemy, timerval), color }
	end
end
emu.registerafter(DisplayObjects)
...
taseditor.submitinputchange(10, 1, 8)
taseditor.submitinputchange(130, 1, 8)
taseditor.applyinputchanges()

FRAMES=ReadTAS(TAS)
while true do
    taseditor.setplayback(120)
    emu.speedmode("maximum")
    for i=0,30,1 do
        memory.writebyte(WorldNumber, 7)
        memory.writebyte(LevelNumber, 1)
        memory.writebyte(0x6274, math.floor((framerule / 1000) % 10))
        memory.writebyte(0x6275, math.floor((framerule / 100) % 10))
        memory.writebyte(0x6276, math.floor((framerule / 10) % 10))
        memory.writebyte(0x6277, math.floor((framerule / 1) % 10))
        FCEU.frameadvance()
    end
    while true do
        FCEU.frameadvance()
        if memory.readbyte(0x770) == 1 and memory.readbyte(0x772) == 3 then
            break
        end
    end
    local at = emu.framecount() - 1
    for i=1,table.getn(FRAMES),1 do
        taseditor.submitinputchange(at + i, 1, FRAMES[i])
    end
    taseditor.applyinputchanges()
    for i=1,table.getn(FRAMES),1 do
        FCEU.frameadvance()
    ...
    local plants = 0
    for enemy=0,5,1 do
        if memory.readbyte(Enemy_ID + enemy) == 0x0d and memory.readbyte(Enemy_X_Position + enemy) == 200 then
            plants = plants + 1
        end
    end
    if memory.readbyte(Player_X_Position) < 100 then
        emu.print(string.format("framerule %d: failed to reach plant", framerule))
    elseif plants == 0 then
        emu.print(string.format("framerule %d: despawned plant", framerule))
    end
